...
def plot_route_with_marker(routeid: str, target_stop_number: str):
    ...
    target_row = df[df['stop_number'].str.strip() == target_stop_number.strip()]
    if not target_row.empty:
        ...
        print(f"⚠️ 找不到指定車站：{target_stop_number}")
...
if __name__ == "__main__":
    ...
    for stop in stops:
        print(f"車站名稱: {stop[2]}, stop_number: {stop[1]}")  # 顯示車站名稱和 stop_number

    target_stop_number = input("請輸入目標車站的 stop_number：").strip()
    plot_route_with_marker(routeid, target_stop_number)